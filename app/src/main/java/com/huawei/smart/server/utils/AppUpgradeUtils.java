...
import com.huawei.smart.server.upgrade.AppUpgrade;
...
public class AppUpgradeUtils {
    ...
    private static final org.slf4j.Logger LOG = LoggerFactory.getLogger(AppUpgradeUtils.class.getSimpleName());
    ...
    public static File getAppFile(AppUpgrade appUpgrade) {
        String appName = getApkName(appUpgrade);
        return new File(appUpgrade.getTargetPath()
                .concat(File.separator + appUpgrade.getNewVersion())
                .concat(File.separator + appName));
    }

    @NonNull
    public static String getApkName(AppUpgrade appUpgrade) {
        String apkUrl = appUpgrade.getApkFileUrl();
    ...
    public static boolean appIsDownloaded(AppUpgrade appUpgrade) {
        //md5不为空
        //文件存在
        //md5只一样
        File appFile = getAppFile(appUpgrade);
        return !TextUtils.isEmpty(appUpgrade.getApkFileMD5())
                && appFile.exists()
                && EncryptUtils.encryptMD5File2String(appFile).equalsIgnoreCase(appUpgrade.getApkFileMD5());
    }
    ...
    public static boolean installApp(Context context, File appFile) {
        try {
            ...
            LOG.error("Failed to open APP store", e);
        }
        return false;
    }
    ...
    public static Intent getInstallAppIntent(Context context, File appFile) {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                ///storage/emulated/0/Android/data/com.huawei.server.manager/cache/1.1.3/sample.apk
                Uri fileUri = FileProvider.getUriForFile(context, context.getApplicationContext().getPackageName(), appFile);
