...
//            channel.setVibrationPattern(new long[]{100, 200, 300, 400, 500, 400, 300, 200, 400});
        ...
        mBuilder.setContentTitle(getString(R.string.upgrade_app_download_start))
            .setContentText(getString(R.string.upgrade_app_download_connect_to_server))
     ...
     * 下载模块
     */
    private void startDownload(AppUpgrade upgrade, final DownloadCallback callback) {
        String apkUrl = upgrade.getApkFileUrl();
        if (TextUtils.isEmpty(apkUrl)) {
            String contentText = "新版本下载路径错误";
            stop(contentText);
            return;
        }
        String appName = AppUpgradeUtils.getApkName(upgrade);

        File appDir = new File(upgrade.getTargetPath());
        if (!appDir.exists()) {
            appDir.mkdirs();
        }

        String target = appDir + File.separator + upgrade.getNewVersion();
        upgrade.getHttpManager().download(apkUrl, target, appName, new FileDownloadCallBack(callback));
    }
    ...
    public class DownloadBinder extends Binder {
         ...
         * @param callback  下载回调
         */
        public void start(AppUpgrade updateApp, DownloadCallback callback) {
            //下载
    ...
    class FileDownloadCallBack implements HttpManager.FileCallback {
        ...
        public void onProgress(float progress, long total) {
            ...
            if (oldRate != rate) {
                ...
                if (mBuilder != null) {
                    mBuilder.setContentTitle(getString(R.string.upgrade_app_download_percent_prefix) + AppUpgradeUtils.getAppName(DownloadService.this))
                        .setContentText(rate + "%")
        ...
        public void onError(String error) {
            Toast.makeText(DownloadService.this, getString(R.string.upgrade_app_download_failed) + error, Toast.LENGTH_SHORT).show();
            //App前台运行
            if (mCallBack != null) {
        ...
        public void onResponse(File file) {
            ...
            try {
                if (AppUpgradeUtils.isAppOnForeground(DownloadService.this) || mBuilder == null) {
                    ...
                    if (mCallBack != null) {
                        intercepted = mCallBack.onInstallAppAndAppOnForeground(file);
                    }
                    ...
                    mBuilder.setContentIntent(contentIntent)
                        ...
                        .setContentText(getString(R.string.upgrade_app_download_finished))
